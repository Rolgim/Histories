export function createPanel({ REGIONS, EVENTS, Theme, MapRenderer, activeSnapshot }) {
  ...
  function tagsHTML(obj) {
    return ["territory", "religion", "language", "ethnicity"]
      .map((k) => `<span class="tag${k === Theme.current ? " tag-active" : ""}">${obj[k]}</span>`)
      .join("");
  }
  ...
  function showEvent(i, cameFromContext) {
    ...
    `;
    backBtn.style.display = lastContext ? "block" : "none";
    open();
    MapRenderer.setSelectedEvent(i);
  }

  function eventRowsHTML(related) {
    return (
      related
        .map(({ e, i }) => `
          <div class="event-row ${e.year > MapRenderer.year ? "future" : ""}" data-i="${i}">
            <div class="event-row-year">An ${e.year}</div>
  ...
  function wireEventRows() {
    body.querySelectorAll(".event-row").forEach((row) => {
      row.addEventListener("click", () => showEvent(parseInt(row.dataset.i, 10), lastContext));
    });
  }

  function showRegion(regionId) {
    lastContext = { type: "region", id: regionId };
    const region = REGIONS.find((r) => r.id === regionId);
    const snap = activeSnapshot(region, MapRenderer.year);
    const related = EVENTS.map((e, i) => ({ e, i }))
      .filter((x) => x.e.regionId === regionId)
      .sort((a, b) => a.e.year - b.e.year);

    body.innerHTML = `
      <div id="panel-year">${snap.territory}</div>
  ...
  function showGroup(mode, value) {
    lastContext = { type: "group", mode, value };
    const year = MapRenderer.year;
    const memberRegions = REGIONS.filter((r) => activeSnapshot(r, year)[mode] === value);
    const related = EVENTS.map((e, i) => ({ e, i }))
      .filter((x) => x.e[mode] === value)
      .sort((a, b) => a.e.year - b.e.year);
    const modeLabel = Theme.MODES.find((m) => m.key === mode).label;

    body.innerHTML = `
      <div id="panel-year">${modeLabel}</div>
  ...
  function showRegionOrGroup(regionId) {
    if (Theme.current === "territory") showRegion(regionId);
      ...
      const region = REGIONS.find((r) => r.id === regionId);
      showGroup(Theme.current, activeSnapshot(region, MapRenderer.year)[Theme.current]);
    }
  }
  ...
  backBtn.addEventListener("click", () => {
    ...
    if (lastContext.type === "region") showRegion(lastContext.id);
    else showGroup(lastContext.mode, lastContext.value);
  });
